    ...
    const handleTutorAvailabilityUpdated = (data: any) => {
      ...
      if (data.action === "removed") {
        setTutorResponses(prev =>
          prev.filter(resp => resp.tutorAddress.toLowerCase() !== data.tutor.address.toLowerCase()),
        );
            ...
            {pendingRequests.map(requestId => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-gray-600 dark:text-gray-400">Waiting for tutor responses...</div>
                </div>
                <button
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border">
          ...
          <div className="space-y-2">
            {tutorResponses.map(response => (
              <div
                key={`${response.requestId}-${response.tutorAddress}`}
                className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded border border-green-200 dark:border-green-800"
              >
                <div>
