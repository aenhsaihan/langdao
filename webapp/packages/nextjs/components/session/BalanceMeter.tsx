...
export const BalanceMeter = ({ current, initial, ratePerSecond, mode = "balance" }: BalanceMeterProps) => {
      ...
      <div className="flex items-center gap-3 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-white backdrop-blur-lg">
        ...
        <div className="flex min-w-32 flex-col gap-1">
          ...
          <div className="h-1.5 w-full rounded-full bg-white/10">
            <div className="h-full rounded-full bg-emerald-300" style={{ width: "100%" }} />
          </div>
        </div>
      </div>
    );
